  ...
  return (
    <div className={clsx(
      ...
      <details className={clsx(
        `mobile:rounded-lg mobile:open:bg-cream-100 ${rigth2} mobile:open:shadow-lg mobile:p-1`,
        ...
        <div className={clsx(
          'mobile:grid mobile:grid-cols-2 mobile:gap-1 mobile:p-2 mobile:text-sm mobile:desktop:border-t-2 mobile:border-greyBlack-100 mobile:text-greyBlack-400 mobile:justify-items-start',
          "desktop:grid-cols-3 desktop:leading-6"
        )}>
          {
            filtros && filtros?.map((e, i) => <button className={clsx('desktop:overflow-hidden desktop:max-h-6 desktop:hover:border-cream-300 desktop:border-cream-100 desktop:hover:bg-cream-300 desktop:duration-500 desktop:w-full','border-b-[0.2px] mobile:hover:border-cream-200 mobile:border-black text-start mobile:w-32')} key={i} id={e.genre || e} onClick={() => handleOnClick(e.genre || e)}>{e.genre || e}</button>)
          }
        </div>
      </details>
    </div>
